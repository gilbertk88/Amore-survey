 ...
 * @return void
 */
function icode_generate_stats_page()
{
	...
	$active_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : 'survey-and-survey-submissions';
	$tab_array = $mlwiCodesurveyMaster->pluginHelper->get_stats_tabs();
	?>
	<div class="wrap">
		<h2><?php _e('survey/Survey Statistics', 'icode-survey-master'); ?></h2>
		<h2 class="nav-tab-wrapper">
			...
			foreach($tab_array as $tab)
			{
				...
				echo "<a href=\"?page=icode_stats&tab=".$tab['slug']."\" class=\"nav-tab $active_class\">".$tab['title']."</a>";
			}
			?>
		</h2>
		<div>
 ...
 * @return void
 */
function icode_stats_overview_tab()
{
	global $mlwiCodesurveyMaster;
	$mlwiCodesurveyMaster->pluginHelper->register_stats_settings_tab(__("Survey Submissions", 'icode-survey-master'), "icode_stats_overview_content");
}
add_action("plugins_loaded", 'icode_stats_overview_tab');
 ...
 * @return void
 */
function icode_stats_overview_content()
{
		...
		<div class="postbox">
			<form action="" method="post">
				<select name="range">
					...
					<?php do_action('icode_survey_taken_stats_options'); ?>
				</select>
 ...
 */
function icode_load_stats($type, $amount = 0) {
	$stats = array();
	switch ($type) {
		case 'daily':
			global $wpdb;
	...
	$stats = apply_filters('icode_survey_taken_stats_load_stats', $stats, $type, $amount);
	return $stats;
}
?>
