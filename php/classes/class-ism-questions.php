...
class iCODE_Questions {
	...
	/**
	 * Loads questions for a survey using the new page system
	 *
	 * @since 5.2.0
	 * @param int $survey_id The ID of the survey.
	 * @return array The array of questions.
	 */
	public static function load_questions_by_pages( $survey_id ) {
		...
		if ( count( $question_ids ) > 0 ) {
			...
			// If we do not have pages on this survey yet, use older load_questions and add page to them.
			$questions = self::load_questions( $survey_id );
	...
	/**
	 * Loads questions for a survey
	 *
	 * @since 5.2.0
	 * @param int $survey_id The ID of the survey.
	 * @return array The array of questions.
	 */
	public static function load_questions( $survey_id ) {
		...
		if ( 0 !== $survey_id ) {
			$survey_id = intval( $survey_id );
			$questions = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}mlw_questions WHERE survey_id=%d AND deleted='0' ORDER BY question_order ASC", $survey_id ), 'ARRAY_A' );
		} else {
	...
	public static function delete_question( $question_id ) {
		...
		if ( false === $results ) {
			...
			$mlwiCodesurveyMaster->log_manager->add( 'Error when deleting question', $msg, 0, 'error' );
	...
	private static function create_save_question( $data, $answers, $settings, $is_creating = true ) {
		...
		if ( false === $results ) {
			$msg = $wpdb->last_error . ' from ' . $wpdb->last_query;
			$mlwiCodesurveyMaster->log_manager->add( 'Error when creating/saving question', $msg, 0, 'error' );
			throw new Exception( $msg );
		}
