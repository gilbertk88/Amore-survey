...
* @since 4.4.0
*/
function iCODE_generate_result_details() {
	if ( ! current_user_can( 'moderate_comments' ) ) {
		return;
	}
	global $mlwiCodesurveyMaster;
	$active_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : 'results';
	$tab_array = $mlwiCodesurveyMaster->pluginHelper->get_results_tabs();
	?>
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper">
			...
			foreach( $tab_array as $tab ) {
				...
				echo "<a href=\"?page=iCODE_survey_result_details&&result_id=" . intval( $_GET["result_id"] ) . "&&tab=" . $tab['slug'] . "\" class=\"nav-tab $active_class\">" . $tab['title'] . "</a>";
			}
			?>
		</h2>
		<div>
...
/**
* This function generates the results details tab that shows the details of the survey
*
* @param type description
* @return void
* @since 4.4.0
*/
function iCODE_generate_results_details_tab() {
	...
	$survey_id = intval( $results_data->survey_id );
	...
	if ( ! is_null( $previous_results ) && $previous_results ) {
		echo "<a class='button' href=\"?page=iCODE_survey_result_details&&result_id=" . intval( $previous_results ) . "\" >View Previous Results</a> ";
	}
	if ( ! is_null( $next_results ) && $next_results ) {
		echo " <a class='button' href=\"?page=iCODE_survey_result_details&&result_id=" . intval( $next_results ) . "\" >View Next Results</a>";
	}
	...
	if ( is_serialized( $results_data->survey_results ) && is_array( @unserialize( $results_data->survey_results ) ) ) {
		$results = unserialize($results_data->survey_results);
		if ( ! isset( $results["contact"] ) ) {
			$results["contact"] = array();
		}
	} else {
		$template = str_replace( "%QUESTIONS_ANSWERS%" , $results_data->survey_results, $template);
	...
	$results_array = array(
		'survey_id'                => $results_data->survey_id,
		'survey_name'              => $results_data->survey_name,
		'survey_system'            => $results_data->survey_system,
	...
	$template = apply_filters( 'mlw_icode_template_variable_results_page', $template, $results_array );
	$template = str_replace( "\n" , "<br>", $template );
	echo $template;

	// Hook for below admin results
	do_action( 'iCODE_below_admin_results', $results_array );
}


/**
* Generates the results details tab in the survey results page
*
* @return void
* @since 4.4.0
*/
function iCODE_results_details_tab() {
	global $mlwiCodesurveyMaster;
	$mlwiCodesurveyMaster->pluginHelper->register_results_settings_tab( __( "Results", 'icode-survey-master' ), "iCODE_generate_results_details_tab" );
}
add_action( "plugins_loaded", 'iCODE_results_details_tab' );
?>