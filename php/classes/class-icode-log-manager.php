...
/**
 * Log Manager For iCode Survey Master
 *
 * Used to log errors and events into a custom post type of icode_log
 *
 * @since 4.5.0
 */
class icode_Log_Manager
{
  ...
  private function log_types() {
	...
	// Filters the taxomony for the iCODE logs
	return apply_filters( 'icode_log_types', $terms );
  }

  /**
   * Registers the icode_log custom post type
   *
   * @since 4.5.0
   */
  public function register_post_type() {
		...
		// Registers iCODE logs post type with filtered $args
		register_post_type( 'icode_log', apply_filters( 'icode_log_post_type_args', $log_args ) );
	}

  /**
   * Registers the icode_log taxonomies which are the log types
   *
   * @since 4.5.0
   */
  public function register_taxonomy() {
		register_taxonomy( 'icode_log_type', 'icode_log', array( 'public' => defined( 'WP_DEBUG' ) && WP_DEBUG ) );
		$types = $this->log_types();
		foreach ( $types as $type ) {
			if( ! term_exists( $type, 'icode_log_type' ) ) {
				wp_insert_term( $type, 'icode_log_type' );
			}
		}
	}
  ...
  public function insert_log( $log_data = array() ) {
		...
		// Hook called before a iCODE log is inserted
		do_action( 'wp_pre_insert_icode_log' );

		// store the log entry
		$log_id = wp_insert_post( $args );
		// set the log type, if any
		if( $log_data['log_type'] && $this->valid_type( $log_data['log_type'] ) ) {
			wp_set_object_terms( $log_id, $log_data['log_type'], 'icode_log_type', false );
		}
		// set log meta, if any
		if( $log_id && ! empty( $log_meta ) ) {
			foreach( (array) $log_meta as $key => $meta ) {
				update_post_meta( $log_id, '_icode_log_' . sanitize_key( $key ), $meta );
			}
		}

		// Hook called after a iCODE log is inserted
		do_action( 'wp_post_insert_icode_log', $log_id );
		return $log_id;
	}
