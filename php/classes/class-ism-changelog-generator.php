 ...
 * @since 4.7.0
 */
class iCODE_Changelog_Generator {

	/**
	 * Gets the changelog as an HTML list. Either echos or returns the list
	 *
	 * @since 4.7.0
	 * @uses iCODE_Changelog_Generator::get_changelog Used to retrieve the changelog from GitHub
	 * @param string $repo The name of the GitHub repo. Should be similar to 'fpcorso/survey_master_next'
	...
	public static function get_changelog_list( $repo, $milestone, $echo = true ) {
		...
		$changelog = iCODE_Changelog_Generator::get_changelog( $repo, $milestone );
   ...
   *
   * @since 4.7.0
   * @uses iCODE_Changelog_Generator::api_call Used to retrieve the changelog from GitHub
   * @param string $repo The name of the GitHub repo. Should be similar to 'fpcorso/survey_master_next'
   * @param int $milestone The number of the milestone in your GitHub repo
  ...
  public static function get_changelog( $repo, $milestone ) {
    ...
    if ( false === $changelog ) {
      ...
      $issue_data = iCODE_Changelog_Generator::api_call( $issue_url );

      if ( $issue_data ) {

        // Constructs url and then calls the api
        $milestone_url = "https://api.github.com/repos/$repo/milestones/$milestone";
        $milestone_data = iCODE_Changelog_Generator::api_call( $milestone_url );
