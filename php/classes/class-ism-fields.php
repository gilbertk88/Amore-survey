...
class iCODE_Fields {
  ...
  public static function generate_section( $fields, $section ) {
    ...
    if ( isset( $_POST["save_settings_nonce"] ) && wp_verify_nonce( $_POST['save_settings_nonce'], 'save_settings') ) {
      ...
      $results = $mlwiCodesurveyMaster->pluginHelper->update_survey_setting( $section, $settings_array );
      if ( false !== $results ) {
  			$mlwiCodesurveyMaster->alertManager->newAlert( __( 'The settings has been updated successfully.', 'icode-survey-master' ), 'success' );
  			$mlwiCodesurveyMaster->audit_manager->new_audit( 'Settings Have Been Edited' );
  		} else {
  			$mlwiCodesurveyMaster->alertManager->newAlert( __( 'There was an error when updating the settings. Please try again.', 'icode-survey-master' ), 'error');
  		}
    }

    // Retrieve the settings for this section
    $settings = $mlwiCodesurveyMaster->pluginHelper->get_survey_setting( $section );

    ?>
    <form action="" method="post">
      <?php wp_nonce_field( 'save_settings','save_settings_nonce' ); ?>
      <button class="button-primary"><?php _e('Save Changes', 'icode-survey-master'); ?></button>
      <table class="form-table" style="width: 100%;">
        ...
        foreach ( $fields as  $field ) {
          ...
          // Generate the field
          iCODE_Fields::generate_field( $field, $settings[ $field["id"] ] );
        }
        ?>
      </table>
      <button class="button-primary"><?php _e('Save Changes', 'icode-survey-master'); ?></button>
  ...
  public static function generate_field( $field, $value ) {
    ...
    iCODE_Fields::$method( $field, $value );
      ...
      <th scope="row">
        <label for="<?php echo $field["id"]; ?>"><?php echo $field["label"]; ?>
          ...
          if ( is_array( $field["variables"] ) ) {
            ?>
            <br>
            <p><?php _e( "Allowed Variables:", 'icode-survey-master' ); ?></p>
            <?php
