...
(function ($) {
  iCODEContact = {
    load : function() {
      $.each( iCODEContactObject.contactForm, function( i, val ) {
        iCODEContact.addField( val );
      });
    },
    addField : function( fieldArray ) {
      ...
      setTimeout( iCODEContact.removeNew, 250 );
    },
    removeNew: function() {
    ...
    duplicateField: function( linkClicked ) {
      ...
      iCODEContact.addField( fieldArray );
    },
    ...
    save : function() {
      iCODEContact.displayAlert( 'Saving contact fields...', 'info' );
      ...
      var data = {
    		action: 'iCODE_save_contact',
    		contact_form: contactForm,
        survey_id : iCODEContactObject.surveyID
    	};

    	jQuery.post( ajaxurl, data, function( response ) {
    		iCODEContact.saved( JSON.parse( response ) );
    	});
    },
    saved : function( response ) {
      if ( response.status ) {
        iCODEContact.displayAlert( '<strong>Success</strong> Your rules have been saved!', 'success' );
      } else {
        iCODEContact.displayAlert( '<strong>Error</strong> There was an error encountered when saving your rules. Please try again.', 'error' );
      }
    },
    displayAlert: function( message, type ) {
      iCODEContact.clearAlerts();
  ...
  $(function() {
    ...
    $( '.contact-form' ).on( 'click', '.delete-field', function( event ) {
      ...
      iCODEContact.deleteField( $( this ) );
    });
    $( '.contact-form' ).on( 'click', '.copy-field', function( event ) {
      event.preventDefault();
      iCODEContact.duplicateField( $( this ) );
    });
